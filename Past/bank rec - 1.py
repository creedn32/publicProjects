...
for col in range(2, 18):
    excelGPTableSheet.Cells(1, col).Value = "G " + emptyStr(excelGPSheet.Cells(1, col - 1).Value)
...
while excelBankSheet.Cells(bankSheetRow, 1).Value:
    ...
    excelBankSheet.Range(excelBankSheet.Cells(bankSheetRow, 1), excelBankSheet.Cells(bankSheetRow, 3)).Copy()
    excelBankTableSheet.Range(excelBankTableSheet.Cells(bankTableSheetRow, 2), excelBankTableSheet.Cells(bankTableSheetRow, 4)).PasteSpecial(Paste=win32com.client.constants.xlPasteValues)

    excelBankTableSheet.Cells(bankTableSheetRow, 5).Value = excelBankSheet.Cells(bankSheetRow, 4).Value[0:200]

    excelBankSheet.Cells(bankSheetRow, 5).Copy()
    excelBankTableSheet.Cells(bankTableSheetRow, 6).PasteSpecial(Paste=win32com.client.constants.xlPasteValues)
