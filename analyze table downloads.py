    ...
    for fileNameOld in os.listdir(filePath):
        ...
        if os.path.getsize(fullPathOld) == 0:
            ...
            if fileNameOld.split("-")[1] != beautifulSoupObject.find("title").contents[0].split(" ")[0]:
                fileNameNew = fileNameOld.replace("-" + fileNameOld.split("-")[1] + "-",
                                                  "-" + beautifulSoupObject.find("title").contents[0].split(" ")[
                                                      0] + "-")
...
while listSheet.Cells(row, col["tickerCol"]).Value:
    ...
    listSheet.Cells(row, 30).Formula = "=IF(AB" + str(row) + "=0,IF(G" + str(row) + "=\"N/A\",I" + str(
        row) + ",G" + str(row) + "),AB" + str(row) + ")"
    listSheet.Cells(row, 31).Formula = "=IF(AND(AD" + str(row) + "<$B$1,AD" + str(row) + "<>0,AC" + str(
        row) + "<$B$2,S" + str(row) + ">0),\"Passed Test\","")"
